        ...
        if doc.GetElement(el_circuit.LookupParameter('БУДОВА_Классификация нагрузок').AsElementId()).Name == 'Освещение':
            ...
            # БУДОВА_Признак цепи (str) отделили коэффициенты стояков от магистралей 
                if 'магистраль' in el_circuit.LookupParameter('БУДОВА_Признак цепи').AsString():
                    st_mag = el_circuit.LookupParameter('БУДОВА_Признак цепи').AsString()[:2]
                    # k_st_mag = st_mag[0] + '.' + st_mag[1]
                    mag.append(float(st_mag[0] + '.' + st_mag[1]))
                elif 'стояк' in el_circuit.LookupParameter('БУДОВА_Признак цепи').AsString():
                    st_styk = el_circuit.LookupParameter('БУДОВА_Признак цепи').AsString()[:2]
                    styk.append(float(st_styk[0] + '.' + st_styk[1]))
            # Длина цепи (Double)
