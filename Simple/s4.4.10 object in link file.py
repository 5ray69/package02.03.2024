...
def get_wall_center_face(wall):
    '''Извлечение центральной плоскости стены в виде объекта класса Face'''
    g_options = DB.Options()
    g_options.IncludeNonVisibleObjects = True
    g_options.ComputeReferences = True
    for g_object in wall.Geometry[g_options]:
        if isinstance(g_object, DB.Solid):
            ...
            # равно 1 и плоскость имеет свойство Reference
...
famil_types = [
    famil_type for famil_type in FEC(arch_link_data['doc']).OfCategory(
...
# for h in hosts:
#     # Возвращает нижние грани этого хоста (типиз. список ссылок IList<Reference>)
#     # вяли из списка первую ссылку
#     reference = HostObjectUtils.GetBottomFaces(h)[0]
#     # Создает ссылку из ссылки в связи RVT (возвращает Reference на объект в связи)
#     linked_reference = reference.CreateLinkReference(linked_instance)

#     parsed_ref = parseLinkedReference(doc, linked_reference)
#     # Получить один геометрический примитив, содержащийся в элементе по ссылке.
