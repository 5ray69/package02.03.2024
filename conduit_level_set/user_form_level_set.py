...
class ErrorMessageViewNotConnectedToLevel(Exception):
    def __init__(self):
...
class UserFormSelectLinkInstance(System.Windows.Forms.Form):
    def __init__(self, listLinkInstanceName):
        self.listLinkInstanceName = listLinkInstanceName

        self.Text = "Выберите имя файла связи"
        ...
        self.MinimumSize = System.Drawing.Size(660, (600 + caption_height))
        self.CenterToScreen()

        self.listUserSelect = []

        self._list_box = System.Windows.Forms.ListBox()
        self._list_box.Size = System.Drawing.Size(500, 400)
        ...
        for nameLink in self.listLinkInstanceName:
            self._list_box.Items.Add(nameLink)
        self._list_box.SelectionMode = System.Windows.Forms.SelectionMode.One
        self.Controls.Add(self._list_box)

        self._label = System.Windows.Forms.Label()
        self._label.Text = 'Щелчок мышью выбирает имя файла связи в списке\
                            \n\
                            \nЕсли ничего не будет выбрано, то файл связи не будет найден\
                            \n\
                            \nЕсли имени связи нет в списке, то связь нужно через правую кнопку мыши Обновить'
