...
# viewSched = [viewSchedule for viewSchedule in FEC(doc).OfClass(DB.ViewSchedule) if "Ключи спецификация кабелей" in viewSchedule.Name][0]

# # получаем Id элемента-строки ключевой спецификации по его имени
# OUT = [el for el in FEC(doc, viewSched.Id).ToElements() if "ВВГнг-LS 3х2,5" in el.Name]
# fromViewSchedule = FromViewSchedule(doc, "Ключи спецификация кабелей")
# fromViewSchedule1 = FromViewSchedule(doc, "Ключи спецификация кабелей")
OUT = [
        FromViewSchedule(doc, "Ключи спецификация кабелей").getElementId("ВВГнг-LS 3х2,5").Name,
        FromViewSchedule(doc, "Ключи спецификация кабелей").getElementId("ПВ1 1х4").Name,
        FromViewSchedule(doc, "Ключи спецификация кабелей").getElementId("FLAME 3х2,5").Name,
        FromViewSchedule(doc, "Ключи спецификация кабелей").getElementId("ПВС 2х0,75").Name,
        FromViewSchedule(doc, "Ключи спецификация кабелей").getElementId("UTP 4x2х0,51").Name
        ]
...
#                 # присваиваем значение 'ПВС 2х0,75' ElementId 8425543
#                 el_circu.LookupParameter('Тип кабеля').Set(DB.ElementId(8425543))
...
#                 # присваиваем значение 'UTP 4x2x0,75' ElementId 2067716
#                 el_circu.LookupParameter('Тип кабеля').Set(DB.ElementId(2067716))
#                 # Если калассификация нагрузок Id=8388542 (Автоматика теплых полов)
#                 el_circu.LookupParameter('БУДОВА_Классификация нагрузок').Set(DB.ElementId(8388542))
