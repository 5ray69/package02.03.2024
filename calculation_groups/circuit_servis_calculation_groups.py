...
from calculation_groups.user_warning_calculation_groups import NoTypeCableIdException
# from user_warning_calculation_groups import NoTypeCableIdException # ДЛЯ ТЕСТА
...
class CircuitСontain(object):
    ...
    def get_dU(self):
        ...
        koef_С = 12
        if self.circuit.LookupParameter('кабСечение').AsString():
            return self.kilovatt_on_meter / koef_С / float(self.circuit.LookupParameter('кабСечение').AsString())
        else:
            raise NoTypeCableIdException(str(self.elementId_circuit.IntegerValue))
