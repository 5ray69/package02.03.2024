...
# clr.AddReference('RevitServices')  # ДЛЯ ТЕСТА Работа с документом и транзакциями 
...
# from user_warning_calculation_groups import ParametrGroupEmptyException, \
#                                 DoubleGroupsException, NoTypeCableIdException
                                
# from circuit_servis_calculation_groups import CircuitСontain, \
...
class CalculationGroups(object):
    ...
    def __init__(self, doc, uidoc):
        ...
        self.all_elidSechenia_in_group = self.get_all_elidSechenia_in_group()
        self.sumLengths_sechenya_in_groups = self.get_sumLengths_sechenya_in_groups()
    ...
    def get_full_path_to_head_panel(self):
        ...
        На выходе словарь из всех имеющихся путей у группы, в каждом пути все цепи.
    ...
    def get_all_elidSechenia_in_group(self):
        '''извлекает уникальные elementId сечениий в каждой группе
        '''
        dict_uniqId_sechenya_in_groups = {}

        for glob_group_str, list_all_circuits in self.circuits_in_groups.items():
            if glob_group_str not in dict_uniqId_sechenya_in_groups:
                dict_uniqId_sechenya_in_groups[glob_group_str] = []

            for one_circuit_from_all in list_all_circuits:
                elementId_type_cable = one_circuit_from_all.LookupParameter('Тип кабеля').AsElementId()

                if elementId_type_cable not in dict_uniqId_sechenya_in_groups[glob_group_str]:
                    dict_uniqId_sechenya_in_groups[glob_group_str].append(elementId_type_cable)

        return dict_uniqId_sechenya_in_groups

    def get_sumLengths_sechenya_in_groups(self):
        '''извлекает сумму всех длин, для каждого elementId сечения (в виде строки), в каждой группе
        '''
        dict_length_sechenya_in_groups = {}

        for glob_group_str, list_all_circuits in self.circuits_in_groups.items():
            if glob_group_str not in dict_length_sechenya_in_groups:
                dict_length_sechenya_in_groups[glob_group_str] = []

            dict_Id_allLength = {}

            for one_circuit_from_all in list_all_circuits:
                elementId_type_cable = one_circuit_from_all.LookupParameter('Тип кабеля').AsElementId()
                if elementId_type_cable.IntegerValue == -1:
                    raise NoTypeCableIdException(str(one_circuit_from_all.Id.IntegerValue))
                keyLength = str(elementId_type_cable.IntegerValue)
                if keyLength not in dict_Id_allLength:
                    dict_Id_allLength[keyLength] = [0]
                circuit_sum = dict_Id_allLength[keyLength].pop() + CircuitСontain(one_circuit_from_all).get_length_up_round()
                dict_Id_allLength[keyLength].append(circuit_sum)

            dict_length_sechenya_in_groups[glob_group_str] = dict_Id_allLength

        return dict_length_sechenya_in_groups
...
# cg = CalculationGroups(doc, uidoc)
# OUT = cg.all_elidSechenia_in_group
# OUT = cg.sumLengths_sechenya_in_groups
# OUT = cg.circuits_in_groups
# OUT = cg.full_path_to_head_panel
# OUT = cg.list_circuits_in_path_with_max_dU